...
- **Frontmatter matches OpenClaw spec:** Only `name`, `description`, `homepage`, and `metadata` (single-line JSON) are used. Fields like `author`, `version`, `permissions`, and `triggers` are NOT part of the spec and cause the skill to be silently dropped by the parser. The `name` uses kebab-case (`max-banking`) to match the repo/directory name and the slash-command invocation (`/max-banking`).
